...
Mirrors the Rust ``workspace/menubar.rs`` module.
"""
...
class MenuCursorFilter(QObject):
    ...
    def eventFilter(self, watched: QObject, event: QEvent) -> bool:  # noqa: N802 - Qt override.
        if (
            isinstance(watched, (QMenu, QMenuBar))
...
def create_menu_bar(window: MainWindow) -> None:  # noqa: PLR0915
    """Build the full menu bar for the main window."""
    ...
    window.toggle_note_debug_action.triggered.connect(
        window.settings_handler.toggle_note_debug_overlay
    )
    ...
    window.toggle_export_btn_action.triggered.connect(
        window.settings_handler.toggle_export_button
    )
